...
const Route = async (
  ...
  usingMiddleware(req, res, async () => {
    const method = await usingMethods(req, ['GET', 'PUT'])
    const gameSessionId = req.query.gameSessionId as string

    // get all feedbacks for this game session
    if (method === 'GET') {
      const feedbacks = await client.gameSession.findUnique({
    ...
    if (method === 'PUT') {
      ...
      const updatedFeedback = await client.feedback.upsert({
        // select model by composite primary key
        where: {
